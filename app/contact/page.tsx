...
const usageCounts=['1회','2회','3회','4회','5회','6회'];
...
export default function Contact(){
  ...
  return <main>
    ...
    <section className="contact-area wrap"><form className="contact-form" onSubmit={submit}><p className="form-kicker">INQUIRY</p><h2>세탁 견적 문의</h2><small>* 표시는 필수 입력 항목입니다</small><input className="website-field" name="website" tabIndex={-1} autoComplete="off" aria-hidden="true"/>
      ...
      <fieldset><legend>이용횟수 (선택)</legend><div className="item-buttons">{usageCounts.map(x=><button type="button" className={usageCount===x?'active':''} key={x} onClick={()=>setUsageCount(v=>v===x?'':x)}>{x}</button>)}</div></fieldset>
      <fieldset><legend>금액 (선택)</legend><div className="item-buttons">{priceTypes.map(x=><button type="button" className={priceType===x?'active':''} key={x} onClick={()=>setPriceType(v=>v===x?'':x)}>{x}</button>)}</div>{priceType==='원하는 금액'&&<label style={{display:'block',marginTop:'16px'}}>원하는 금액<input name="desiredPrice" inputMode="numeric" placeholder="예: 월 300,000원"/></label>}{priceType==='기존에 하고 있는 금액'&&<label style={{display:'block',marginTop:'16px'}}>현재 이용 중인 금액<input name="currentPrice" inputMode="numeric" placeholder="예: 월 250,000원"/></label>}</fieldset>
      <label>문의내용<textarea name="message" rows={6} placeholder="현재 이용 중인 세탁에서 불편한 점이나 개선되었으면 하는 부분을 자유롭게 적어주세요."/></label><label className="agree"><input type="checkbox" required/> 개인정보 수집 및 이용과 FormSubmit을 통한 문의 정보 전달에 동의합니다.</label><button className="submit-btn" type="submit" disabled={status==='sending'}>{status==='sending'?'전송 중...':'문의 보내기'}</button>{status==='success'&&<p className="form-status success" role="status">문의가 접수되었습니다. 15분 이내로 대표자가 직접 연락 드리겠습니다.</p>}{status==='error'&&<p className="form-status error" role="alert">메일 전송에 실패했습니다. 잠시 후 다시 시도해 주세요.</p>}
    ...
    <footer className="contact-footer"><div className="legal wrap"><span>상호 조세탁연구소　|　대표자: 조성집　|　사업자등록번호: 877-31-01664</span><span>ⓒ 조세탁연구소. All rights reserved.</span></div></footer><div className="floating"><a href="https://bit.ly/4vHlBz0" target="_blank" rel="noopener noreferrer" aria-label="조세탁연구소 카카오톡 문의" style={{order:-2,width:'156px',height:'44px',borderRadius:'999px',background:'#FEE500',color:'#191919',fontSize:'13px',fontWeight:700,display:'flex',gap:'8px',padding:'0 16px'}}><span aria-hidden="true">💬</span><span>카카오톡 문의하기</span></a><a href="https://blog.naver.com/house_90" target="_blank" rel="noopener noreferrer" aria-label="조세탁연구소 네이버 블로그"><span className="social-icon" aria-hidden="true">N</span><span>조세탁연구소</span></a><a href="https://www.youtube.com/channel/UCeeyW2Jew63mU9BHacpsu4A" target="_blank" rel="noopener noreferrer" aria-label="조세탁연구소 유튜브" style={{order:-1}}><svg width="24" height="24" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M23.5 6.2a3 3 0 0 0-2.1-2.1C19.5 3.6 12 3.6 12 3.6s-7.5 0-9.4.5A3 3 0 0 0 .5 6.2 31 31 0 0 0 0 12a31 31 0 0 0 .5 5.8 3 3 0 0 0 2.1 2.1c1.9.5 9.4.5 9.4.5s7.5 0 9.4-.5a3 3 0 0 0 2.1-2.1A31 31 0 0 0 24 12a31 31 0 0 0-.5-5.8ZM9.6 15.6V8.4L15.8 12l-6.2 3.6Z"/></svg><span>조세탁연구소</span></a><a href="tel:01035550517">☎　010-3555-0517</a></div>
  </main>
}
