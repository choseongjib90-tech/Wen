...
export default function Contact(){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    ...
    setStatus('sending');
    try{
      const response=await fetch('https://formsubmit.co/ajax/whtod13@naver.com',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({_subject:'[조세탁연구소] 새로운 세탁 견적 문의',_template:'table',_honey:fd.get('website'),성함:fd.get('name'),연락처:fd.get('phone'),'매장 지역':fd.get('area'),업종:fd.get('business')||'미선택','하루 발생량':fd.get('volume')||'미선택',세탁물:selected.join(', ')||'미선택','금액 기준':priceType||'미선택','원하는 금액':fd.get('desiredPrice')||'미입력','기존 이용 금액':fd.get('currentPrice')||'미입력',문의내용:fd.get('message')||'없음'})});
